...
describe('<CourseListRow />', () => {
    it("When isHeader is true, a check to test the component renders one cell with colspan = 2 when textSecondCell does not exist",() =>{
        ...
        expect(wrapper.html()).to.equal(`<tr class="row_21x1yy" style="background-color:#deb5b545"><th colSpan="2">${props.children.props.children}</th></tr>`);
    });
    ...
    it("When isHeader is true, a check to test the component renders two cells when textSecondCell is present",() =>{
        ...
        expect(wrapper.html()).to.equal(`<tr class="row_21x1yy" style="background-color:#deb5b545"><th>${props.children[0].props.children}</th><th>${props.children[1].props.children}</th></tr>`);
    });
